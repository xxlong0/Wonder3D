...
#         self._b_relative_laplace = b_relative_laplace
#
#         V = self._vertices.shape[0]
#         # prepare continuous tensor for all vertex-based data
#         if not b_relative_laplace:
#             self._vertices_etc = torch.zeros([V, 9], device=vertices.device)
#             self._split_vertices_etc()
#         else:
#             self._vertices_etc = torch.zeros([V, 12], device=vertices.device)
#             self._split_vertices_etc()
#             self._initial_pos.copy_(vertices)
#
#         # self._split_vertices_etc()
...
#         self._smooth = self._vertices_etc[:, :8] if with_gammas else self._vertices_etc[:, :3]
#
#         if self._b_relative_laplace:
#             self._initial_pos = self._vertices_etc[:, 9:12]
#
#     def zero_grad(self):
#         self._vertices.grad = None
#
...
#             torch_scatter.scatter_mean(src=relative_pos.flip(dims=[1]).reshape(E * 2, -1), index=edges.reshape(E * 2),
#                                        dim=0, out=relative_neightbor_smooth)
#
#             # apply optional smoothing of m1,m2,nu
#         if self._gammas[0]:
#             self._m1.lerp_(neighbor_smooth[:, 5:8], self._gammas[0])
...
#
#         # add laplace smoothing to gradients
#         if not self._b_relative_laplace:
#             laplace = self._vertices - neighbor_smooth[:, :3]  # 平均值
#         else:
#             laplace = self._vertices - self._initial_pos - relative_neightbor_smooth
#         #     print("laplace: ", laplace.sum())
#         # print("origin grad: ", self._vertices.grad)
#         grad = torch.addcmul(self._vertices.grad, laplace, self._nu[:, None], value=self._laplacian_weight)
#
#         # gradient clipping
#         if self._step > 1:
...
#
#     @torch.no_grad()
#     def laplace_smooth(self, vertices=None):
#
#         if vertices is None:
#             vertices = self._vertices
#
#         # spatial smoothing
#         edges, _ = calc_edges(self._faces)  # E,2 #应该是每个边的两个顶点的索引
#         E = edges.shape[0]
#         edge_smooth = vertices[edges]  # E,2,S ， 顶点特征映射为边的特征
#         neighbor_smooth = torch.zeros_like(vertices)  # V,S
#         # 计算每个顶点相邻的所有特征的均值
#         torch_scatter.scatter_mean(src=edge_smooth.flip(dims=[1]).reshape(E * 2, -1), index=edges.reshape(E * 2, 1),
#                                    dim=0, out=neighbor_smooth)
#         vertices = neighbor_smooth
#
#         return vertices
#
#     @torch.no_grad()
#     def update_relative_laplace(self):
#         import copy
#         if self._b_relative_laplace:
#             vertices_smoothed = self._vertices.detach().contiguous()
#             for _ in range(1):
#                 vertices_smoothed = self.laplace_smooth(vertices=vertices_smoothed)
#             self._initial_pos.copy_(vertices_smoothed)
#             # self._initial_pos.copy_(self._vertices.detach().contiguous())
